...
sequenceDiagram
    ...
    Server-->>Client: 2. 402 Payment Required<br/>(amount, token, network, payTo)
    Client->>Client: 3. Sign EIP-3009<br/>transferWithAuthorization
    Client->>Server: 4. GET /resource<br/>X-PAYMENT: {signed payload}
    Server->>Facilitator: 5. /verify (check signature,<br/>amount, funds)
    Facilitator-->>Server: ✅ Valid
    Server->>Server: 6. Perform work<br/>(inference, query, etc.)
    Server->>Facilitator: 7. /settle
    Facilitator->>Chain: Submit signed authorization
    Chain-->>Facilitator: Tokens transferred
    Server-->>Client: 8. 200 OK + result<br/>X-PAYMENT-RESPONSE: receipt
```
